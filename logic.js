...
function renderTable() {
    ...
    for (let rIdx = startIdx; rIdx < endIdx; rIdx++) {
        ...
        orderedHeaders.forEach(key => {
            ...
            let maxLen = rule.max || '';
            if (key === 'cellphone_no') maxLen = '13'; // 12 + optional quote
            if (key === 'tin_no' || key === 'pin_no') maxLen = '17';
            if (key === 'bin') maxLen = '19';
            if (rule.numeric && !maxLen) maxLen = '20';
            
            let htmlAttr = maxLen ? ` maxlength="${maxLen}"` : '';
            html += `<td><input type="text" class="${cls}" data-row="${rIdx}" data-col="${key}" value="${escapeHtml(displayVal)}"${htmlAttr}></td>`;
        });
        html += '</tr>';
    }
    ...
    c.querySelectorAll('.cell-input').forEach(input => {
        input.addEventListener('input', e => {
            ...
            if (rule.numeric) {
                ...
                e.target.value = e.target.value.replace(/[^\d']/g, '');
            } else if (k === 'tin_no' || k === 'pin_no' || k === 'bin') {
                e.target.value = e.target.value.replace(/[^\d-]/g, '');
            } else if (k === 'incharge_extension_name') {
                e.target.value = e.target.value.replace(/\./g, '');
            } else if (k === 'location_owned') {
